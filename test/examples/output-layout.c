...
static void animate_cat(struct local_server *sample,
  ...
  bool ur_collision = !wlr_output_layout_output_at(
      sample->layout, sample->x_offs + 128, sample->y_offs);
  bool ul_collision = !wlr_output_layout_output_at(
      sample->layout, sample->x_offs, sample->y_offs);
  bool ll_collision = !wlr_output_layout_output_at(
      sample->layout, sample->x_offs, sample->y_offs + 128);
  bool lr_collision = !wlr_output_layout_output_at(
      sample->layout, sample->x_offs + 128, sample->y_offs + 128);
...
static void output_frame_notify(struct wl_listener *listener, void *data) {
  ...
  struct wlr_render_pass *pass =
      wlr_output_begin_render_pass(wlr_output, &output_state, NULL, NULL);
...
static void update_velocities(struct local_server *sample, float x_diff,
                              float y_diff) {
  sample->x_vel += x_diff;
  sample->y_vel += y_diff;
}
...
int main(int argc, char *argv[]) {
  ...
  state.cat_texture = wlr_texture_from_pixels(
      state.renderer, DRM_FORMAT_ABGR8888, cat_tex.width * 4, cat_tex.width,
      cat_tex.height, cat_tex.pixel_data);
