    ...
    case XKB_KEY_P:
      if (fork() == 0) {
        execl("/bin/sh", "/bin/sh", "-c",
              "rofi -i -show drun -show-icons -matching fuzzy", (void *)NULL);
      }
      break;
    default:
      return false;
...
static void server_cursor_motion(struct wl_listener *listener, void *data) {
  ...
  wlr_log(WLR_ERROR, "server_cursor_motion: x=%lf,y=%lf", event->delta_x,
          event->delta_y);
...
static void server_cursor_motion_absolute(struct wl_listener *listener,
  ...
  // wlr_log(WLR_ERROR, "server_cursor_motion_absolute: x=%lf,y=%lf", event->x,
  // event->y);
...
static void output_request_state(struct wl_listener *listener, void *data) {
  ...
  wlr_scene_rect_set_size(server->background, output->wlr_output->width,
...
int main(int argc, char *argv[]) {
  ...
  server.background = wlr_scene_rect_create(&server.scene->tree, 1000, 600,
                                            (float[]){0.2, 0.4, 0.6, 1.0});
