...
static bool handle_keybinding(struct tinywl_server *server, xkb_keysym_t sym)
{
	...
	case XKB_KEY_P:
		if (fork() == 0)
		{
			execl("/bin/sh", "/bin/sh", "-c", "rofi -i -show drun -show-icons -matching fuzzy", (void *)NULL);
		}
		break;
	default:
		return false;
...
static void server_cursor_motion(struct wl_listener *listener, void *data)
{
	...
	wlr_log(WLR_ERROR, "server_cursor_motion: x=%lf,y=%lf", event->delta_x, event->delta_y);
